...
<head>
    ...
    <style>
            ...
            /* Make table data visually match header labels (senada) */
            td, td .small {
                color: var(--muted);
                font-size: 13px;
            }
        ...
        .btn.secondary {
            background: #fff;
            color: var(--primary);
            border: 1px solid rgba(11, 118, 239, .15)
        }

        .btn.ghost {
            background: transparent;
            color: var(--primary);
            border: 1px dashed rgba(11, 118, 239, .15)
        }
                ...
                <table id="table">
                    ...
                    <tbody>
                        ...
                        @forelse ($data as $patient)
                            <tr data-name="{{ strtolower($patient['nama'] ?? $patient->name ?? '') }}"
                                data-rm="{{ strtolower($patient['no_ktp'] ?? $patient->no_ktp ?? '') }}"
                                data-phone="{{ strtolower($patient['no_telp'] ?? $patient->no_telp ?? $patient->phone ?? '') }}"
                                data-active="{{ (strtolower($patient['status'] ?? $patient->status ?? '') === 'active' || strtolower($patient['status'] ?? $patient->status ?? '') === 'aktif') ? 'active' : 'inactive' }}">
                                <td>{{ $i++ }}</td>
                                <td>{{ $patient['nama'] ?? $patient->name ?? '-' }}</td>
                                <td class="small">{{ $patient['jenis_kelamin'] ?? $patient->jenis_kelamin ?? '-' }}</td>
                                <td class="small">{{ $patient['golongan_darah'] ?? $patient->golongan_darah ?? '-' }}</td>
                                <td class="small">{{ $patient['tgl_masuk'] ?? $patient->tgl_masuk ?? '-' }}</td>
                                <td class="small">{{ $patient['no_kamar'] ?? $patient->no_kamar ?? '-' }}</td>
                                <td class="small">{{ $patient['status'] ?? $patient->status ?? '-' }}</td>
                                <td class="small">{{ \Illuminate\Support\Str::limit($patient['diagnosa'] ?? $patient['deskripsi'] ?? $patient->diagnosa ?? '-', 80) }}</td>
                            </tr>

                        @empty
                            <tr>
                                <td colspan="13" class="empty">Belum ada data pasien.</td>
                            </tr>
                        @endforelse
                        </tbody>
                    </table>
        ...
        <script>
            (function() {
                ...
                function applySort() {
                    ...
                    // sort cards DOM
                    const cards = Array.from(cardsContainer.querySelectorAll('.card')).filter(c => c.style.display !==
                        'none');
                    cards.sort((a, b) => {
                ...
                document.getElementById('btnSync').addEventListener('click', () => {
                    ...
                    b.disabled = true;
                    b.textContent = 'Menyinkron...';
                    setTimeout(() => {
                        b.disabled = false;
                        b.textContent = 'Sinkronisasi';
                        alert('Sinkronisasi contoh selesai.');
                    }, 1100);
                });
